...
- `ruyi_human_drag` 提供原子的拟人拖拽动作链；`ruyi_set_fingerprint.windowSize` 可同步 window、viewport 与 screen 指标。
...
- `ruyiPage==1.2.50`，以及由 ruyiPage 安装或兼容的 Firefox。
...
- ruyiPage `1.2.50` 的安装器仍使用 `151-ruyi` runtime；如需验证 credentialed HTTP / SOCKS5 proxy，应单独下载上游 [`151-proxy`](https://github.com/LoseNine/ruyipage/releases/tag/151-proxy) release，并通过 `RUYI_FIREFOX_PATH` 显式指向解压后的 `firefox.exe`。
- 在 `151-proxy` 上同步窗口尺寸时，`browsingContext.setViewport` 可能等待 3 秒后由 ruyiPage 自动切换到 JS viewport fallback；这是兼容路径，不影响后续浏览器复用。
